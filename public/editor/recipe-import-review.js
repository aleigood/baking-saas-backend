(function (window) {
    ...
    const keyOf = (value) => normalizeName(value).replace(/[\s·・_-]+/g, '').toLocaleLowerCase('zh-CN');
    ...
    function createIngredientReviewItems(result, catalogItems) {
        ...
        return collectIngredientCandidates(result).map((ingredient, index) => {
            const sourceName = normalizeName(ingredient?.sourceName || ingredient?.normalizedName || `原料${index + 1}`);
    ...
    function walkIngredients(result, visitor) {
        asArray(result?.recipes).forEach((recipe) => {
            asArray(recipe?.versions).forEach((version) => {
                asArray(version?.ingredients).forEach((ingredient) => visitor(ingredient, recipe, version, 'ingredients'));
                asArray(version?.products).forEach((product) => {
                    asArray(product?.mixIn).forEach((ingredient) => visitor(ingredient, recipe, version, 'mixIn', product));
                    asArray(product?.mixIns).forEach((ingredient) => visitor(ingredient, recipe, version, 'mixIns', product));
                    asArray(product?.fillings).forEach((ingredient) => visitor(ingredient, recipe, version, 'fillings', product));
                    asArray(product?.toppings).forEach((ingredient) => visitor(ingredient, recipe, version, 'toppings', product));
                });
            });
        });
    }
    ...
    function mergeIngredientList(items) {
        ...
        asArray(items).forEach((item) => {
            ...
            if (!name) return;
            const amountKind = hasValue(item.flourRatio) ? 'flourRatio' : hasValue(item.weightInGrams) ? 'weightInGrams' : 'ratio';
