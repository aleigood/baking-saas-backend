...
export class RecipeImportService implements OnModuleInit {
    ...
    private async recoverInterruptedJobs() {
        try {
            ...
            this.logger.error(`智能导入任务恢复失败：${error instanceof Error ? error.message : 'unknown error'}`);
        }
    }
    ...
    private convertModelResult(modelResult: ModelImportResult): {
        ...
        if (Array.isArray(modelResult.intermediateRecipes)) {
            const convertedReviewItems: RecipeImportReviewCandidate[] = [
                ...(modelResult.reviewItems ?? []).filter(
                    (item) => !this.isSecondaryIntermediateAmountReview(item, modelResult.intermediateRecipes ?? []),
                ),
            ];
            ...
            const recipes = modelResult.intermediateRecipes
                ...
                .flatMap((recipe, recipeIndex) =>
                    this.convertIntermediateRecipeBundle(recipe, recipeIndex, convertedReviewItems),
                )
    ...
    private isSecondaryIntermediateAmountReview(
        item: RecipeImportReviewCandidate,
        ...
        const section = intermediateRecipes[recipeIndex]?.sections?.[sectionIndex];
    ...
    private parseIntermediateAmount(item: ModelIntermediateIngredient): {
        ...
        if (typeof item.amount === 'number' && Number.isFinite(item.amount)) {
            ...
            if (this.isGramUnit(normalizedUnit))
                return { sourceValue: sourceValue || String(item.amount), grams: item.amount, unit: normalizedUnit };

            return {
                sourceValue: sourceValue || String(item.amount),
                unit: normalizedUnit,
                reason: `单位 ${normalizedUnit} 不能自动换算为克。`,
            };
        }
    ...
    private parseYieldText(text: string): { unitWeight?: number; yieldCount?: number; totalWeight?: number } {
        ...
        const yieldCount =
            text.match(/[（(]\s*(\d+(?:\.\d+)?)\s*个\s*[）)]/)?.[1] || text.match(/均分\s*(\d+(?:\.\d+)?)\s*个/)?.[1];
    ...
    private mergeDuplicateIngredients<
        T extends { name: string; ratio?: number; flourRatio?: number; importMeta?: Record<string, unknown> },
    >(ingredients: T[]): T[] {
                ...
                version.products?.forEach((product) => {
                    const visit = (items: Array<{ name: string }> | undefined, section: string) =>
                        items?.forEach((ingredient) => {
                            ingredient.name =
                                collect(ingredient.name, `${recipe.name} / ${product.name} / ${section}`) ||
                                ingredient.name;
                        });
