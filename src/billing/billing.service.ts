...
export class BillingService {
    ...
    async getSubscription(tenantId: string) {
        return this.entitlements.getSummary(tenantId);
    }

    async createOrder(userId: string, tenantId: string, tenantRole: TenantRole, planId: string) {
        if (tenantRole !== TenantRole.OWNER) throw new ForbiddenException('只有店主可以购买或续订套餐');
        const [plan, user, entitlementPolicy] = await Promise.all([
            ...
            this.entitlements.getDefaultPolicyForTier(EntitlementTier.PRO),
        ]);
        if (!plan) throw new NotFoundException('套餐不存在或已下架');
        if (!this.wechatPay.isMockMode() && !user?.wechatOpenId)
            throw new BadRequestException('WECHAT_BINDING_REQUIRED');
        ...
        try {
            ...
            await this.prisma.paymentOrder.update({
                ...
                data: {
                    ...
                    failureReason: error instanceof Error ? error.message.slice(0, 500) : '微信下单失败',
                },
            });
            throw error;
        }
    ...
    async syncOrder(tenantId: string, orderNo: string) {
        ...
        const transaction = await this.wechatPay.queryOrder(orderNo);
        ...
        await this.prisma.$transaction(
            async (tx) => {
                const order = await tx.paymentOrder.findUnique({
                    ...
                    include: { plan: true, tenant: { select: { trialEndsAt: true } }, entitlementPolicy: true },
                });
                ...
                if (transaction.amount?.total !== order.amountInCents)
                ...
                const startsAt = [latest?.expiresAt, order.tenant.trialEndsAt]
                    .filter((date): date is Date => Boolean(date && date > now))
                    .reduce((latestDate, date) => (date > latestDate ? date : latestDate), now);
                const expiresAt = new Date(startsAt.getTime() + order.plan.durationDays * 24 * 60 * 60 * 1000);
                const subscription = await tx.tenantSubscription.create({
                    data: {
                        tenantId: order.tenantId,
                        planId: order.planId,
                        entitlementPolicyId: order.entitlementPolicyId,
                        startsAt,
                        expiresAt,
    ...
    async createRefund(orderId: string, amountInCents: number, reason?: string) {
        ...
        if (order.status !== PaymentOrderStatus.PAID && order.status !== PaymentOrderStatus.PARTIALLY_REFUNDED)
    ...
    async handleRefundNotification(headers: Record<string, string | string[] | undefined>, rawBody: string) {
        this.wechatPay.verifyNotification(
            this.header(headers, 'wechatpay-timestamp'),
            this.header(headers, 'wechatpay-nonce'),
            this.header(headers, 'wechatpay-signature'),
            rawBody,
        );
        const envelope = JSON.parse(rawBody) as WechatNotificationEnvelope;
        const data = this.wechatPay.decryptNotification<WechatRefundNotification>(envelope.resource);
        await this.applyRefundResult(data);
        return { code: 'SUCCESS', message: '成功' };
    }

    async applyRefundResult(data: WechatRefundNotification) {
        const refund = await this.prisma.paymentRefund.findUnique({
            where: { refundNo: data.out_refund_no },
            include: { order: true },
        });
        ...
        await this.prisma.$transaction(async (tx) => {
            await tx.paymentRefund.update({
                ...
                data: {
                    ...
                    successAt: data.success_time ? new Date(data.success_time) : new Date(),
                    wechatRefundId: data.refund_id,
                },
            });
            ...
            await tx.paymentOrder.update({
                ...
                data: {
                    refundAmountInCents: refunded,
                    status: full ? PaymentOrderStatus.REFUNDED : PaymentOrderStatus.PARTIALLY_REFUNDED,
                    refundedAt: full ? new Date() : null,
                },
            });
            if (full && refund.order.subscriptionId)
                await tx.tenantSubscription.update({
                    where: { id: refund.order.subscriptionId },
                    data: { status: SubscriptionStatus.CANCELED, notes: '关联订单已全额退款' },
                });
        });
    }
    ...
    async reconcilePendingOrders() {
        const orders = await this.prisma.paymentOrder.findMany({
            where: { status: { in: [PaymentOrderStatus.PENDING, PaymentOrderStatus.FAILED] } },
        ...
        for (const order of orders) {
            try {
                const transaction = await this.wechatPay.queryOrder(order.orderNo);
                await this.applyWechatTransaction(transaction);
                synced += 1;
            } catch {
